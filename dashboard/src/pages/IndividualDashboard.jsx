...
const IndividualDashboard = () => { 
  ...
  // Combined API call: Geolocation (ipwho.is) and Risk Analysis (local backend)
  const analyzeIP = async () => {
    ...
    try {
      ...
      if (geoResponse.data && geoResponse.data.success) {
        ...
        setError(
          geoResponse.data.message || "Failed to fetch geolocation data."
        );
      }
    } catch (err) {
    ...
    try {
      const riskResponse = await axios.post(
        "http://localhost:4000/api/ip/analyze",
        { ip }
      );
  ...
  // Export both geoData and riskData to a combined CSV
  const exportToCSV = () => {
    try {
      const csvRows = [];
  ...
  // Provide a description for each feature key
  ...
  // Helper function to render a simple value
  ...
  // New helper function to format complex objects (flag, connection, timezone)
    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-[radial-gradient(circle,_#000000_20%,_#003300_80%)] text-[#00ff00]">
      ...
      <main className="flex flex-col items-center p-6">
        ...
        <section className="bg-[#001100] text-[#00ff00] rounded-lg shadow-lg w-11/12 my-8 p-8">
          <h2 className="animate-glitch text-center text-2xl mb-6">
            Enter IP Address
          </h2>
